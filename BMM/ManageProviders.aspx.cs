...
namespace BMM
{
    public partial class ManageProviders : Classes.BasePage
    {
        #region +Properties
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                Title = Company.Name + " - Manage Providers";
                CheckUserPermission();
            }
        }
        #endregion
        ...
        {
            Response.Redirect("/AddEditProvider.aspx");
        }
        #endregion
        ...
        /// Binds the datagrid
        /// </summary>
        protected void grdProviders_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            grdProviders.DataSource = (from a in ProviderClass.GetProvidersByCompanyID(CurrentUser.CompanyID, true)
                                       orderby a.Name
                                       select a).ToList();
        }
        #endregion

        #region grdProviders_OnItemCommand
        //upon clicking the name of the provider, or edit, the onitemcommand will load the textbox with the name for editing
        //it will also change out the add button into two buttons for save and cancel
        protected void grdProviders_OnItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs args)
        {
            if (args.CommandName == "btnEdit")
                Response.Redirect("/AddEditProvider.aspx?id=" + args.CommandArgument.ToString());
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(rcbProviderSearch.SelectedValue, out id))
            {
                Response.Redirect("/AddEditProvider.aspx?id=" + id);
            }
            else
                rcbProviderSearch.ClearSelection();
        }
        #endregion

        #region rcbProviderSearch_ItemsRequested
        //loads the combo box with the Provider names
        protected void rcbProviderSearch_ItemsRequested(object sender, Telerik.Web.UI.RadComboBoxItemsRequestedEventArgs e)
        {
            string text = e.Text.ToLower().Trim();
            if (text.Length > 2)
            {
                foreach (var item in (from a in ProviderClass.GetProvidersByNameSearch(text, CurrentUser.CompanyID, true)
                                      select new Telerik.Web.UI.RadComboBoxItem(a.Name, a.ID.ToString())))
                {
                    rcbProviderSearch.Items.Add(item);
                }
            }
        }
        #endregion
        ...
        #region GetAddress
        //gets the address of the corresponding Provider
        public String GetAddress(Provider a)
        {
        ...
        #region GetDelete
        /// <summary>
        /// Either returns an image for the user to delete a provider or returns an inactive image
        /// </summary>
