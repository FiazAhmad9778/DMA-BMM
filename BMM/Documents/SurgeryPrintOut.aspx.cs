...
{
    public partial class SurgeryPrintOut : Classes.BasePage
    {
        ...
        private void BuildPDF()
        {
            ...
            transform.Load(Server.MapPath(XMLPath) + "SurgeryPrintOut.xsl");
            ...
            Page.Response.Clear();
            Page.Response.AddHeader("Content-Disposition", "attachment; filename=SurgeryPrintOut.pdf");
        ...
        private string SerializeToXML()
        {
            ...
            var surgeries = (from s in Invoice.Surgeries
                             where s.Active
                             select SurgeryClass.GetSurgeryByID(s.SurgeryID));
            foreach (var surgery in surgeries)
            {
                xml.Append("<Name>" + Server.HtmlEncode(surgery.Name) + "</Name>");
            }
            foreach (var date in (from surgery in Invoice.Surgeries
                                  where surgery.Active
                                  from date in surgery.SurgeryDates
            ...
            foreach (var provider in (from provider in Invoice.Providers where provider.Active select provider))
            {
                xml.Append("<Provider>");
                {
                    InvoiceProvider ip = !provider.InvoiceProviderID.HasValue ? null : GetInvoiceProvider(provider.InvoiceProviderID.Value);
                    ...
                    {
                        Provider p = GetProvider(provider.ProviderID);
                        if (p != null)
                        {
                    ...
                    decimal totalCost = 0;
                    foreach (var cost in (from s in provider.ProviderServices where s.Active select s.Cost))
                    {
                        xml.Append("<Cost>" + cost.ToString("c") + "</Cost>");
                        totalCost += cost;
                    }
                    decimal totalPPODiscount = (from s in provider.ProviderServices
                                                where s.Active
                                                select s.PPODiscount).Sum();
                    xml.Append("<TotalPPODiscount>" + totalPPODiscount.ToString("c") + "</TotalPPODiscount>");
                    xml.Append("<Total>" + (totalCost - totalPPODiscount).ToString("c") + "</Total>");
                }
                xml.Append("</Provider>");
            }
            xml.Append("</Providers>");
